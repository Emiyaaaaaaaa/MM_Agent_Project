...
### 2.1 架构设计思路
- 整个流程以 `Supervisor` 为调度中心，根据当前阶段状态决定下一步进入哪个节点。
- 每个节点只处理自己负责的任务（读题、分析、建模、实验、审查、写作、导出），职责边界清晰，便于维护。
- 节点之间不是一次性串行执行，而是支持“审查后回流”，让分析、建模、实验和写作可以迭代打磨。

### 2.2 为什么采用这种流转方式
- 数模论文本质上是反复修订过程，不是一次生成即可定稿，因此需要循环机制而不是单向流水线。
- 通过回流节点，系统可以在发现论证不足、实验不充分或图文不一致时，返回对应环节继续完善。
- 这种架构更符合真实竞赛写作节奏：先形成初稿，再按问题逐轮补强，直到达到可提交标准。

### 2.3 运行层能力（LangGraph 落地）
- 基于有状态图执行，支持阶段状态记录、中断恢复和任务内重开。
- 支持节点级进度回传，用户可见当前流转位置和每轮迭代结果。
- 与结构化内容协议结合后，流程结果可回放、可追踪、可持续优化。
...
- 当前主模型路线：**Gemini**。
- 后续将新增开发一个模式：基于 **Gemini Deep Research** 的研究型写作流程。
